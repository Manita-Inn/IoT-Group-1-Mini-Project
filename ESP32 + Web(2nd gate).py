...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .btn-open2 { background: var(--accent2); color: #fff; }
  .btn-close2 { background: var(--surface2); color: var(--warn); border: 1px solid rgba(255,157,0,.25); }
...
<div class="grid">
  ...
  <div class="card">
    <div class="card-label">Parking Gate (Entry)</div>
...
<div class="section-title">Exit Gate (Gate 2)</div>
<div class="card">
  <div class="card-label">Exit Gate Status</div>
  <div id="gate2-badge" class="gate-badge closed"><span class="gate-dot"></span><span id="gate2-text">CLOSED</span></div>
  <div class="sub" style="margin-top:6px">IR sensor auto-controls when not overridden</div>
</div>
...
<div class="section-title">Gate 1 — Entry</div>
<div class="btn-row">
  <button class="btn btn-open"  id="btn-open"  onclick="gateCmd('open')">OPEN GATE 1</button>
  <button class="btn btn-close" id="btn-close" onclick="gateCmd('close')">CLOSE GATE 1</button>
</div>
<div class="section-title">Gate 2 — Exit</div>
<div class="btn-row">
  <button class="btn btn-open2"  id="btn-open2"  onclick="gate2Cmd('open2')">OPEN GATE 2</button>
  <button class="btn btn-close2" id="btn-close2" onclick="gate2Cmd('close2')">CLOSE GATE 2</button>
</div>
...
function setAllBtns(disabled){
  ['btn-open','btn-close','btn-open2','btn-close2'].forEach(function(id){
    document.getElementById(id).disabled=disabled;
  });
}
function gateCmd(cmd){
  clearTimeout(timer); setAllBtns(true);
  fetch('/cmd?action='+cmd)
    .then(function(r){return r.json();})
    .then(function(d){showToast(d.message||'Done');fetchStatus();})
    .catch(function(){showToast('Error');fetchStatus();});
}
function gate2Cmd(cmd){
  clearTimeout(timer); setAllBtns(true);
  fetch('/cmd?action='+cmd)
    .then(function(r){return r.json();})
    .then(function(d){showToast(d.message||'Done');fetchStatus();})
...
function updateUI(d){
  ...
  var g2Open=d.gate2_open,badge2=document.getElementById('gate2-badge'),dot2=badge2.querySelector('.gate-dot');
  badge2.className='gate-badge '+(g2Open?'open':'closed');
  dot2.className='gate-dot '+(g2Open?'open':'');
  document.getElementById('gate2-text').textContent=g2Open?'OPEN':'CLOSED';
  var dist=d.distance;
...
def build_json():
    ...
    return ('{{"available":{a},"total":{t},"gate_open":{g},"gate2_open":{g2},"distance":{d},"slots":[{s0},{s1},{s2}],"temp":{tm},"hum":{h},"ip":"{ip}"}}').format(
        a=get_available(), t=TOTAL_SLOTS,
        g="true" if gate_open else "false",
        g2="true" if gate2_open else "false",
...
def handle_web():
    ...
    global ir_gate_manual, ir_last_state, ir_waiting_close, ir_clear_time
    try:
        ...
        if path == '/' or path == '/index.html':
            ...
            # ---- Gate 2 (Exit) — check first ----
            if 'action=open2' in path:
                ir_servo.duty(IR_DUTY_OPEN)
                ir_gate_manual   = True
                ir_last_state    = "detected"
                ir_waiting_close = False
                msg = "Gate 2 OPENED"
                print("Web: Gate 2 OPENED")

            elif 'action=close2' in path:
                ir_servo.duty(IR_DUTY_CLOSE)
                ir_gate_manual   = False
                ir_last_state    = "clear"
                ir_waiting_close = False
                ir_clear_time    = None
                msg = "Gate 2 CLOSED"
                print("Web: Gate 2 CLOSED")

            # ---- Gate 1 (Entry) — unchanged ----
            elif 'action=open' in path:
                if get_available() == 0:
