...
def handle_ir_servo():
    ...
    # If manually overridden, skip auto IR logic
    if ir_gate_manual:
        return
                ...
                if r.status_code == 200:
                    ...
                    for msg in data.get("result", []):
                        ...
                        if message_data and "text" in message_data:
                            ...
                            # ---- GATE 2 (Exit) — check first to avoid /open matching /open2 ----
                            if "/open2" in text:
                                ir_servo.duty(IR_DUTY_OPEN)
                                ir_gate_manual   = True
                                ir_last_state    = "detected"
                                ir_waiting_close = False
                                send_msg("Gate 2 (Exit) OPENED via Telegram")
                                ...
                                ir_waiting_close = False
                                ir_clear_time    = None
                                send_msg("Gate 2 (Exit) CLOSED via Telegram")
                            ...
                            # ---- GATE 1 (Entry) — unchanged ----
                                ...
                                if get_available() == 0:
                                    ...
                                    last_detect_time = time.ticks_ms()
                                    full_notified    = False
                                    send_msg("Gate 1 (Entry) OPENED via Telegram")

                            elif "/close" in text:
                                set_gate(DUTY_CLOSE)
                                gate_open        = False
                                last_detect_time = None
                                send_msg("Gate 1 (Entry) CLOSED via Telegram")
                            ...
                            # ---- Info commands — unchanged ----
                                ...
                                a = get_available()
                                gate2_str = "OPEN" if (ir_gate_manual or ir_last_state == "detected") else "CLOSED"
                                send_msg("Gate 1 (Entry): {}\nGate 2 (Exit):  {}\nAvailable: {}/{}\nIP: {}".format(
